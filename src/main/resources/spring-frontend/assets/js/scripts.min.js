...
(function ($) {
  ...
  $(function () {
    ...
    var language = function (){
      return{
        init: function (){
          $('.image-ru-flag').on('click', function (){
            window.location.replace('?lang=ru');
          });

          $('.image-en-flag').on('click', function (){
      ...
      return {
        init: function () {
          $('[data-refreshshow]').on('click change', function (e) {
              ...
              $.ajax({
                ...
                complete: function (result) {
                  if (result.status === 200) {
                    ...
                    var totalBookCount = result.responseJSON.count;
                    if ((($this.data('refreshoffset') + 1) * $this.data('refreshlimit')) >= totalBookCount) {
            ...
            function getOffset(tegNameOffset, tegNameLimit) {
              var oldOffset = $this.data(tegNameOffset);
              if (oldOffset < 1) {
                $this.data(tegNameOffset, ++oldOffset);
              } else {
                $this.data(tegNameOffset, ++oldOffset - $this.data(tegNameLimit));
              }
              return $this.data(tegNameOffset);
            }
            ...
            switch (data) {
                ...
                if (e.type === 'change') {
                  ...
                  if (!$this.hasClass('form-input_date_uninit')) {
                    ...
                    getData('/api/books/recent', {
                      ...
                      offset: getOffset('refreshoffset', 'refreshlimit'),
              ...
              case'viewed':
                ...
                break;
              case'genre':
                getData('/api/books/genre/' + $this.data('refreshparam'), {
                  offset: getOffset('refreshoffset', 'refreshlimit'),
                  limit: $this.data('refreshlimit')
                });
                break;
              case'author':
                getData('/api/books/author/' + $this.data('refreshparam'), {
                  offset: getOffset('refreshoffset', 'refreshlimit'),
                  limit: $this.data('refreshlimit')
                });
                break;
              case'tag':
                getData('/api/books/tag/' + $this.data('refreshparam'), {
                  offset: getOffset('refreshoffset', 'refreshlimit'),
                  limit: $this.data('refreshlimit')
                });
                break;
              case'search':
                getData('/api/search/' + $this.data('refreshparam'), {
                  offset: getOffset('refreshoffset', 'refreshlimit'),
                  limit: $this.data('refreshlimit')
                });
                break;
      ...
      return {
        init: function () {
          ...
          $('#submitPhone').on('click', function () {
            ...
            postData('/api/requestContactConfirmation', data, function (result) {
              if (result.result) {
          ...
          $sendButton.on('click', function (e) {
            ...
            if ($this.is('[data-send]')) {
              ...
              if (!error) {
                postData('/api/requestLoginContactConfirmation', $this.data('send'), function (result) {
                  if (result.result) {
          ...
          $entryButton.on('click', function (e) {
            ...
            if ($this.is('[data-send]')) {
              ...
              if (!error) {
                postData('/api/approveContact', $this.data('send'), function (result) {
                  if (result.result) {
    ...
    var Profile = function () {
      ...
      return {
        init: function () {
          ...
          $saveProfile.find('input[name="mail"], input[name="phone"]').on('focusin', function(){
            $(this).data('val', $(this).val());
          });

          $saveProfile.find('input[name="mail"], input[name="phone"]').on('change', function () {
            var prev = $(this).data('val');
            var current = $(this).val();
            if(!prev || prev === current){
              return;
            }
            var $this = $(this), $row = $this.closest('.form-group'), $code = $row.find('.form-input_code');
            $row.find('.form-info').hide();
            $row.find('.Profile-btn_confirm').show();
            $row.find('.form-error').remove();
            $this.removeClass('form-input_error');
            $code.hide();
            $code.prop('disabled', 'disabled');
            $code.val('');
            $code.removeClass('form-input_error');
            formChange = true;
          });

          $('#profileConfirmPhone').on('click', function () {
            var $this = $(this), $row = $this.closest('.form-group'),
              data = {contact: $row.find('.form-input').val()};
            Login().postData('/api/requestContactConfirmation', data, function (result) {
              if (result.result) {
                $row.find('#phone').after('<div class="form-info">Вам направлен код подтверждения по SMS</div>');
                $row.find('.Profile-btn_confirm').hide();
                $row.find('.form-input_code').show();
                $row.find('.form-input_code').prop('disabled', false);
                $row.find('.form-error').remove();
              } else {
                $row.find('.form-input[name="phone"]').addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
              }
            })
          });

          $('#profileConfirmMail').on('click', function () {
            var $this = $(this), $row = $this.closest('.form-group'),
              data = {contact: $row.find('.form-input').val()};
            Login().postData('/api/requestContactConfirmation', data, function (result) {
              if (result.result) {
                $row.find('#mail').after('<div class="form-info">Вам направлен код подтверждения по EMail</div>');
                $row.find('.Profile-btn_confirm').hide();
                $row.find('.form-input_code').show();
                $row.find('.form-input_code').prop('disabled', false);
                $row.find('.form-error').remove();
              } else {
                $row.find('.form-input[name="mail"]').addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
              }
            })
          });

          $('#btnCancelProfile').on('click', function (){
            window.location.replace("/profile/cancel");
          });

          $saveProfile.on('submit', function (e){
            if(!formChange || !$('.form-info_confirm_mail').is(':visible') ||
              !$('.form-info_confirm_phone').is(':visible')){
              e.preventDefault();
            }
          });

          $saveProfile.find('.form-input_code').on('blur', function () {
            var $this = $(this), $row = $this.closest('.form-group'), data = {
              contact: $row.find('.form-input_mail, .form-input_phone').val(),
              code: $row.find('.form-input_code').val()
            };
            if (!$this.hasClass('form-input_error')) {
              Login().postData('/api/approveContact', data, function (result) {
                if (result.result) {
                  $row.find('.form-info').hide();
                  $row.find('.form-input_code').hide();
                  $row.find('.form-input_code').prop('disabled', 'disabled');
                  $row.find('.form-info_confirm').show();
                } else {
                  $this.addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                }
              })
      ...
      return {
        init: function () {
          $('form.Topup').on('submit', function (e) {
            var $this = $(this);
            e.preventDefault();
            var sumVal = $this.find('[name="sum"]').val();
            if (!sumVal) {
              return;
            }
    ...
    var Transactions = function () {
      var transactionTemplate = function (transaction) {
        ...
        var transValue = transaction.value > 0 ? '+' + transaction.value : transaction.value;
        return '<tr>\n' + '<td>' +
          date.getDate() + ' ' +
          (month[date.getMonth()]) + ' ' +
          date.getFullYear() + ' ' +
        ...
        $.ajax({
          ...
          complete: function (result) {
            if (result.status === 200) {
              ...
              var curCount = ($('.Transactions-get').data('transactionoffset') + 1) *
                $('.Transactions-get').data('transactionlimit');
              if (result.responseJSON.count <= curCount) {
                $('.Transactions-footer').hide();
              }
